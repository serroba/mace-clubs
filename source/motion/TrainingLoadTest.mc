...
function testTrainingLoadIgnoresSessionsOutsideTheChronicWindow(logger as Test.Logger) as Boolean {
    var now = 2000000000;
    var log = [] as Array<Storage.ValueType>;
    log = SmoothnessLog.append(
        log,
        MotionTestFixtures.trainingLoadTestRecord(now - 40 * TRAINING_LOAD_TEST_DAY, 5000)
    );
    Test.assertMessage(
...
function testTrainingLoadSteadyWeekMatchesBaseline(logger as Test.Logger) as Boolean {
    ...
    for (var i = 0; i < 4; i++) {
        log = SmoothnessLog.append(
            log,
            MotionTestFixtures.trainingLoadTestRecord(now - i * 8 * TRAINING_LOAD_TEST_DAY, 10000)
        );
    }
...
function testTrainingLoadSpikeAboveBaselineReadsHigh(logger as Test.Logger) as Boolean {
    ...
    log = SmoothnessLog.append(
        log,
        MotionTestFixtures.trainingLoadTestRecord(now - 21 * TRAINING_LOAD_TEST_DAY, 2000)
    );
    log = SmoothnessLog.append(
        log,
        MotionTestFixtures.trainingLoadTestRecord(now - 28 * TRAINING_LOAD_TEST_DAY, 2000)
    );
    log = SmoothnessLog.append(log, MotionTestFixtures.trainingLoadTestRecord(now, 40000));
    var ratio = TrainingLoad.acuteChronicRatio(log, now) as Float;
...
function testTrainingLoadQuietWeekAfterABusyMonthReadsLow(logger as Test.Logger) as Boolean {
    ...
    log = SmoothnessLog.append(
        log,
        MotionTestFixtures.trainingLoadTestRecord(now - 14 * TRAINING_LOAD_TEST_DAY, 30000)
    );
    var ratio = TrainingLoad.acuteChronicRatio(log, now) as Float;
