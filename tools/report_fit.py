...
<style>
  :root {{ color-scheme: light dark; --bg:#f7f5f1; --surface:#fff; --ink:#24211d; --muted:#6f6961; --grid:#d9d4cc; --work:#e66b38; --rest:#4d7ca8; --rms:#bd3e14; --peak:#e4a02b; --hr:#b32747; --smooth:#397a68; --warn:#ad561f; }}
  @media (prefers-color-scheme:dark) {{ :root {{ --bg:#171614; --surface:#211f1c; --ink:#f3eee7; --muted:#b8afa4; --grid:#49443e; --work:#f1814f; --rest:#6c9ccc; --rms:#ff7848; --peak:#ffc052; --hr:#ff698b; --smooth:#67c4a9; --warn:#ff9a55; }} }}
  * {{ box-sizing:border-box; }} body {{ margin:0; background:var(--bg); color:var(--ink); font:15px/1.45 system-ui,-apple-system,sans-serif; }}
  ...
  @media (max-width:700px) {{ main {{ padding:20px 12px 40px; }} .metrics {{ grid-template-columns:repeat(2,minmax(0,1fr)); }} }}
</style>
</head>
<body><main>
...
const root=document.documentElement, summary=report.summary, records=report.records, laps=report.laps;
...
function drawTimeline(){{
  ...
  laps.forEach(d=>{{for(const box of [motionBox,hrBox]){{const r=S('rect',{{x:x(d.start),y:box.t,width:Math.max(1,x(d.end)-x(d.start)),height:box.b-box.t,class:d.phase==='work'?'phase-work':'phase-rest'}});svg.append(r);}}}});
