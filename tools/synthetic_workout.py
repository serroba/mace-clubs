#!/usr/bin/env python3
"""Generate deterministic motion vectors, a FIT activity, and a visual oracle."""
...
GENERATED_MONKEY_TEST = ROOT / "source/SyntheticMotionVectorsTest.mc"
...
def render_monkey_test(windows: list[Window]) -> str:
    representatives = [windows[0], windows[5], windows[30], windows[42]]
    lines = [
        "import Toybox.Lang;", "import Toybox.Test;", "", "// Generated by tools/synthetic_workout.py; do not edit.", "",
    ]

    def array(name: str, values: tuple[int, ...]) -> list[str]:
        return [f"    var {name} = ["] + [
    ...
    for window in representatives:
        label = window.style.capitalize()
        lines.extend(["(:test)", f"function testSynthetic{label}MotionVector(logger as Test.Logger) as Boolean {{"])
        lines.extend(array("x", window.x))
        lines.extend(array("y", window.y))
        lines.extend(array("z", window.z))
        lines.extend([
            "    var result = Motion.features(x, y, z);",
            f'    Test.assertEqualMessage(result[:rms] as Number, {window.rms}, "{window.style} RMS matches independent oracle");',
            f'    Test.assertEqualMessage(result[:peak] as Number, {window.peak}, "{window.style} peak matches independent oracle");',
            f'    Test.assertEqualMessage(result[:zc] as Number, {window.crossings}, "{window.style} crossings match independent oracle");',
            f'    Test.assertEqualMessage(result[:dynamicRms] as Number, {window.dynamic_rms}, "{window.style} dynamic RMS matches independent oracle");',
            f'    Test.assertEqualMessage(result[:dynamicPeak] as Number, {window.dynamic_peak}, "{window.style} dynamic peak matches independent oracle");',
            "    return true;", "}",
        ])
    return "\n".join(lines) + "\n"
...
def generated_outputs(directory: Path) -> dict[Path, str | bytes]:
    ...
    return {
        ...
        directory / "SyntheticMotionVectorsTest.mc": render_monkey_test(windows),
    }


def monkey_tokens(source: str) -> str:
    """Ignore formatter-only whitespace while preserving generated semantics."""
...
def main(argv=None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="mace-synthetic-") as temporary:
        ...
        monkey = next(value for path, value in outputs.items() if path.suffix == ".mc")
        if args.check:
            failures = []
            if not BASELINE.exists() or BASELINE.read_text() != svg:
                failures.append(f"visual baseline differs: regenerate {BASELINE}")
            if not GENERATED_MONKEY_TEST.exists() or monkey_tokens(GENERATED_MONKEY_TEST.read_text()) != monkey_tokens(monkey):
                failures.append(f"Monkey C vectors differ: regenerate {GENERATED_MONKEY_TEST}")
            if failures:
                raise SystemExit("\n".join(failures))
